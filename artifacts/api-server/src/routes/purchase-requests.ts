...
const PET_PURCHASE_BONUS_RULES = [
  ...
  { characterId: "daifugo", minLevel: 10, source: "level_reward" as const, label: "大富豪 Lv.10報酬", rate: 5, eventOnly: false },
  { characterId: "daifugo", minLevel: 30, source: "level_reward" as const, label: "大富豪 Lv.30報酬", rate: 5, eventOnly: false },
] as const;
...
function getSlotMaxRebateRate(unlockedSlots: number, baseRebateRate: number, isEventDay: boolean): number {
  return baseRebateRate + getMaxPetRebateBonus(unlockedSlots, isEventDay);
}
...
router.get("/purchase-requests", requireAuth, async (req, res): Promise<void> => {
  ...
  try {
    ...
    res.json({
      ...
      scheduledSlotMaxRebateRate: getSlotMaxRebateRate(unlockedSlots, getSlotBaseRebateRate(unlockedSlots), false),
