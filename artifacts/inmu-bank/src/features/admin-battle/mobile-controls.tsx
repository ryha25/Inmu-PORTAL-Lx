...
export function MobileControls({ onMove, onLook, onAttack, onUltimate, onDodge }: {
  ...
  return (
    <div className="absolute inset-0 z-30 md:hidden" style={{ touchAction: 'none' }}>
      ...
      <div
        ref={stickRef}
        className="absolute bottom-[max(22px,env(safe-area-inset-bottom))] left-5 size-32 rounded-full border border-white/35 bg-black/35"
        onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); moveStick(event.clientX, event.clientY) }}
      ...
      <div className="absolute bottom-[max(22px,env(safe-area-inset-bottom))] right-4 grid grid-cols-2 gap-3">
