...
import {
  ...
  RefreshCw, CheckCircle2, Info, KeyRound,
} from 'lucide-react'
...
export function AdminProfilePage() {
  ...
  const [codeChangeType, setCodeChangeType] = useState<'owner' | 'operator'>('owner')
  ...
  async function handleChangeCode() {
    if (!currentCode || !newCode) { toast.error('全項目を入力してください'); return }
    if (newCode.length < 6) { toast.error('新しいコードは6文字以上にしてください'); return }
    ...
    try {
      const res = await fetch('/api/admin/change-admin-code', {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: codeChangeType, currentCode, newCode }),
      })
      const d = await res.json() as { ok?: boolean; error?: string }
      if (!res.ok || !d.ok) { toast.error(d.error ?? '変更に失敗しました'); return }
    ...
    <AdminShell onLogout={handleLogout}>
      ...
      <div className="flex flex-col gap-4 max-w-md">
        ...
        <Card className="border-border bg-card p-4">
          ...
          <div className="flex flex-col gap-3">
            <div>
              <Label className="text-xs text-muted-foreground mb-1.5 block">変更対象</Label>
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => setCodeChangeType('owner')}
                  className={`flex-1 rounded-lg border px-3 py-2 text-xs font-medium transition-colors ${codeChangeType === 'owner' ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:border-primary/50'}`}
                >
                  オーナー (A)
                </button>
                <button
                  type="button"
                  onClick={() => setCodeChangeType('operator')}
                  className={`flex-1 rounded-lg border px-3 py-2 text-xs font-medium transition-colors ${codeChangeType === 'operator' ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:border-primary/50'}`}
                >
                  オペレーター (B)
                </button>
              </div>
            ...
            <div>
              <Label className="text-xs text-muted-foreground mb-1.5 block">現在のコード</Label>
              <Input
                type="password"
            ...
            <div>
              <Label className="text-xs text-muted-foreground mb-1.5 block">新しいコード（6文字以上）</Label>
              <Input
                type="password"
                placeholder="新しい管理コード"
                value={newCode}
            ...
            <Button
              ...
              disabled={codeChanging || !currentCode || !newCode}
              className="w-full gap-2 min-h-10"
            >
              <KeyRound className="size-3.5" />
              {codeChanging ? '変更中…' : 'コードを変更する'}
            </Button>
          </div>
        </Card>
