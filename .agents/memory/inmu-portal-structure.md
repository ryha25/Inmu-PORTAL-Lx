...
## Pet page slot-panel UI conventions (固有スキル発動 / レベル報酬効果発動)
As of 2026-07-05, `pet-page.tsx` has 3 independent character-selection concepts, each with its own state and UI, and they must NOT be cross-wired:
1. **Display/training character** (`selectedPetId` from `usePetState`) — which owned pet is shown in the room and interacted with. Derived as `ownedPetIds.includes(selectedPetId) ? selectedPetId : ownedPetIds[0]`. Picked via `CharacterSelectStrip` (square icon row, driven by `ownedPetIds`, NOT `activePetIds`).
2. **固有スキル発動** (`skillActiveCharacterIds`, max 3) — `SkillActivationGrid`/`SkillActivationButton`.
3. **レベル報酬効果発動** (`activePetIds`, up to `unlockedSlots`) — `LevelRewardEffectGrid`/`LevelRewardEffectButton`. Purely a reward-multiplier slot list, unrelated to what's displayed on screen.
All three use the same `grid grid-cols-3` + `aspect-square` tile convention with a button (opens a dialog containing the grid) rather than an inline always-visible panel.
**Why:** the original design conflated (1) and (3) — `activePetIds` was both "training display slots" and "reward slots" — which the user explicitly asked to split apart while keeping the underlying reward mechanics unchanged.

## Gotcha: don't let ownership-sync effects auto-populate a "slots" array
A `useEffect` that runs on ownership load (`GET /api/pet/characters`) previously did `setActivePetIds(owned.slice(0, unlockedSlots))` whenever the current active list was empty, so simply owning a character auto-filled reward slot 1 on first page load — this only became visible/wrong after decoupling display-selection from `activePetIds` (caught by e2e test, not by tsc).
**Why:** that auto-fill logic made sense back when `activePetIds` doubled as "what's currently shown", but once a separate ownership-driven `CharacterSelectStrip` took over display selection, the same auto-fill silently kept mutating the (now purely reward-effect) slot array as a side effect of an unrelated fetch.
**How to apply:** when splitting a single array/state that served two purposes into two separate concerns, grep every `set<Array>(...)` call site for the old array (not just the render usages) — sync effects tied to unrelated events (ownership fetch, mount, etc.) are easy to miss and only surface via e2e testing against real data, not type-checking.
