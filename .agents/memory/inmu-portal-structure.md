...
## Pet page slot-panel UI conventions (固有スキル発動 / レベル報酬スキル発動)
Both slot panels in `pet-page.tsx` (`SkillActivationPanel` for 固有スキル発動, `TrainingSlotsV2` for レベル報酬スキル発動) use a `grid grid-cols-3` layout with `aspect-square` image tiles per slot (changed 2026-07-04 from a vertical flex-col stack with fixed-size circular icons) — keep new slot-style UI consistent with this square-tile grid convention rather than reintroducing vertical rows.
`TrainingSlotsV2` takes a `displayedPetId` prop (computed in `PetPage()` as `activePetIds.includes(selectedPetId) ? selectedPetId : activePetIds[0]`) and renders a "表示中" badge on the slot matching it, or a "この子を画面に表示" button (calls the existing `onView`/`handleSelect`) on other filled slots — this is how the currently-displayed training character is switched from the training list itself.
**Why:** user asked for an explicit switch control in the training list (rather than relying on an implicit whole-row click) since the display-selection state wasn't visible anywhere in that list.
